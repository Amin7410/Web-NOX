...
-- =========================================================================
-- Migration: V3__init_engine_schema.sql
-- Description: Initializes Multi-Tenancy and the Core Engine (Blocks, Workspaces) schema.
-- =========================================================================

-- Table: organizations
-- Purpose: Highest-level Tenant administration and logical isolation of billing and projects.
...
-- Table: roles
-- Purpose: Defines RBAC (Role-Based Access Control) permission matrices scoped to a specific Organization.
CREATE TABLE roles (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Table: org_members
-- Purpose: M-to-M link mapping Users to Organizations with a specific Role assignment.
...
-- Table: projects
-- Purpose: Top-level domain boundary for workflows and canvases, scoped under an Organization.
...
-- Table: workspaces
-- Purpose: Distinct sub-environments (e.g., individual un-scoped Canvases) existing within a Project.
...
-- Table: core_blocks
-- Purpose: The fundamental node entity on the NOX Canvas. Utilizes JSONB for dynamic schema-less Visual/Config states.
...
-- Purpose: Tracks instantiation mappings of Invader Templates injected into specific Core Block instances.
...
-- Purpose: Defining directed edges (wires/connections) bridging source and target Core Blocks on the Canvas.
...
-- Purpose: Immutably captures entire Project state into JSONB data-dumps for Version History and State Rollbacks.
