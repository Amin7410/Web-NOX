...
public class OrgMemberService {
    ...
    public OrgMember addMember(AddMemberCommand command) {
        ...
        User user = userRepository.findByEmail(command.email())
                .orElseThrow(() -> new DomainException("USER_NOT_FOUND", "User not found to add to org"));

        User inviter = userRepository.findByEmail(command.inviterEmail())
                .orElseThrow(() -> new DomainException("USER_NOT_FOUND", "Inviter tracking user not found"));

        if (orgMemberRepository.existsByOrganizationIdAndUserId(command.orgId(), user.getId())) {
            throw new DomainException("ALREADY_MEMBER", "User is already a member of this organization");
        }
        ...
        OrgMember inviterMember = orgMemberRepository.findByOrganizationIdAndUserId(command.orgId(), inviter.getId())
                .orElseThrow(() -> new DomainException("UNAUTHORIZED", "Inviter must be a member of the organization"));

        if (!inviterMember.canAssignRole(role)) {
            throw new DomainException("INSUFFICIENT_PRIVILEGE", "You cannot assign a role with a higher level than your own");
        }
    ...
    public void removeMember(@AuditTargetOrg UUID orgId, UUID userId) {
        ...
        if (member.getRole().getName().equals("OWNER")) {
            ...
            if (ownerCount <= 1) {
                throw new DomainException("CANNOT_REMOVE_LAST_OWNER", "Cannot remove the last owner of an organization.");
            }
        }
